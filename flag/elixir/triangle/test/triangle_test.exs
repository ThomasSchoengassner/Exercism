defmodule TriangleTest do
  ...
  # @tag :pending
  test "triangle has zero area and looks like a single line" do
    assert Triangle.kind(3, 5, 8) == {:ok, "degenerate triangle"}
    assert Triangle.kind(4.3, 3.7, 0.6) == {:ok, "degenerate triangle"}
    assert Triangle.kind(-1, 1, 0) == {:error, "all side lengths must be positive"}
  end
end
